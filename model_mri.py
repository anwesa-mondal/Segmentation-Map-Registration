...
if __name__ == "__main__":
    ...
    # Create dummy data
    B, D, H, W = 1, 128, 128, 128
    template_mri = torch.randn(B, 1, D, H, W).to(device)
    template_seg = torch.randn(B, 5, D, H, W).softmax(dim=1).to(device)
    ...
    # Warp template segmentation
    warped_seg = stn(template_seg, final_flow)
    
    print(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")
    print(f"Final flow shape: {final_flow.shape}")
    print(f"Intermediate flows: {len(intermediate_flows)}")
    print(f"Lambda maps: {len(lambda_maps)}")
    print(f"Attention maps: {len(attention_maps)}")
    print(f"Warped seg shape: {warped_seg.shape}")
