...
if __name__ == "__main__":
    ...
    # Create dummy data
    B, D, H, W = 2, 64, 64, 64
    warped_mri = torch.randn(B, 1, D, H, W).to(device)
    sample_mri = torch.randn(B, 1, D, H, W).to(device)
    warped_seg = torch.randn(B, 5, D, H, W).softmax(dim=1).to(device)
    sample_seg = torch.randn(B, 5, D, H, W).softmax(dim=1).to(device)
    final_flow = torch.randn(B, 3, D, H, W).to(device) * 0.1
    
    intermediate_flows = [
        torch.randn(B, 3, D//8, H//8, W//8).to(device) * 0.1,
        torch.randn(B, 3, D//4, H//4, W//4).to(device) * 0.1,
        torch.randn(B, 3, D//2, H//2, W//2).to(device) * 0.1,
        final_flow
    ]
    
    lambda_maps = [
        torch.ones(B, 1, D//8, H//8, W//8).to(device) * 0.5,
        torch.ones(B, 1, D//4, H//4, W//4).to(device) * 0.5,
        torch.ones(B, 1, D//2, H//2, W//2).to(device) * 0.5,
        torch.ones(B, 1, D, H, W).to(device) * 0.5
    ]
    
    # Compute loss
    loss_fn = MRIRegistrationLoss()
    total_loss, loss_dict = loss_fn(
        warped_mri, sample_mri, warped_seg, sample_seg,
        final_flow, intermediate_flows, lambda_maps, return_components=True
    )
    ...
    for name, value in loss_dict.items():
        ...
        print(f"{name:20s}: {v:.6f}")
    print("-" * 50)
    print(f"{'Total Loss':20s}: {total_loss.item():.6f}")
